...
- **连击得分**：2.2 秒内持续击破累积连击，击杀得分倍率最高 ×3；中弹清零、炸弹击杀照常计入，结算页展示击破数 / 最大连击 / 用时
...
**连击**：2.2 秒内连续击破敌机即累积连击，击杀得分按 `1 + 连击数 × 0.1` 加成（封顶 ×3）。中弹清零；炸弹击杀照常计入（把保命资源换成连击爆发的博弈）；Boss 击破是保底分、不进连击。
